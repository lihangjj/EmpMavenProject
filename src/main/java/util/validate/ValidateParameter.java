        ...
        try {
            ...
            if (rule != null) {    // 可以应用规则
                ...
                for (int x = 0; x < result.length; x++) {    // 开始取出每一个规则进行检测
                    ...
                    switch (temp[1]) {
                        case "string": {
                            if (!validateEmpty(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.string.msg"));
                            }
                            break;
                        }
                        case "int": {
                            if (!validateInt(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.int.msg"));
                            }
                            break;
                        }
                        case "double": {
                            if (!validateDouble(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.double.msg"));
                            }
                            break;
                        }
                        case "date": {
                            if (!validateDate(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.date.msg"));
                            }
                            break;
                        }
                        case "datetime": {
                            if (!validateDatetime(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.datetime.msg"));
                            }
                            break;
                        }
                        default: {    // 如果不再以上的范围之内的操作就属于验证码的处理
                            if (!validateEmpty(val)) {    // 验证失败
                                errors.put(temp[0], servlet.getMsg("validation.string.msg"));
                            } else {    // 现在验证码不为空，那么就需要进行验证码是否合法的检测
                                String rand = (String) servlet.getSession().getAttribute("rand");
                                if (rand == null) {    // 表示现在没有验证码
                                    errors.put(temp[0], servlet.getMsg("validation.rand.msg"));
                                } else {
